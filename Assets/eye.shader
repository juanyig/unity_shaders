...
float3 L_tmp = normalize(i.L);
				...
				float3 diffu = tex2D (_MainTex, i.uv) * _MainColor.xyz;
				...
				float sinTH, dirAtten, sinTH2, dirAtten2, spec2;
			    	...
			    	float lightIntensity =  (dirAtten2 * pow(sinTH2, _HighlightPower/10)  > 0.13 && SHADOW_ATTENUATION(i)> 0.99)? 1 : 0;
